...
9. **Sécurisation & Backup Automatique de la Base de Données (Nouveau)**
   ...
   - **Sauvegarde Cloud via Firebase Storage (Nouveau) :** Ajout d'une section "Sauvegarde Cloud (Firebase)" dans les Paramètres, juste sous la sauvegarde locale, permettant d'envoyer/récupérer la même sauvegarde JSON consolidée (produits, ventes, dettes, réappros, mouvements de caisse) vers/depuis un bucket Firebase Cloud Storage. Implémenté via un client REST léger (`FirebaseBackupManager`, OkHttp) plutôt que le SDK Firebase ou le plugin Gradle `google-services`, afin que la compilation ne soit jamais bloquée par une configuration Firebase absente. Le nom du bucket se configure directement dans l'écran (aucune reconstruction de l'application nécessaire), sur le même principe que la saisie manuelle d'IP du module de synchronisation.
...
15. **Sauvegarde Cloud Firebase (Nouveau)**
    - **`FirebaseBackupManager`** : client REST léger (OkHttp, déjà une dépendance du projet) pour l'API Firebase Cloud Storage — aucune dépendance au SDK Firebase ni au plugin Gradle `google-services`, donc la compilation ne peut jamais être cassée par une configuration Firebase absente ou invalide.
    - Section "Sauvegarde Cloud (Firebase)" dans Paramètres, sous la sauvegarde locale existante : le bucket Firebase Storage se configure directement dans l'écran (`AppPreferences.firebaseStorageBucket`), sans rebuild nécessaire. Réutilise `getFullDatabaseJsonSync()`/`syncFullDatabaseSync()` déjà en place, donc le format de sauvegarde Cloud reste strictement identique à la sauvegarde locale et à la synchronisation réseau.
